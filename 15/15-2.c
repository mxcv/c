/*ќрган≥зувати базу даних з ≥нформац≥Їю про сп≥вроб≥тник≥в установи,
використавши структуру з наступними пол€ми:
Ц ѕ≤Ѕ;
Ц дата народженн€(дд:мм:рррр); // ISO-8601 (YYYY-MM-DD) is better
Ц стать;
Ц р≥чний дох≥д;
Ц к≥льк≥сть д≥тей.
ƒодатковий серв≥с: вивести на екран в≥дсортовану за пр≥звищем ≥нформац≥ю
про сп≥вроб≥тник≥в установи, що народилис€ в зазначений час року.*/
...
void save(struct Employee* arr, int size);
void load(struct Employee** arr, int* size, int* max_size);
...
int main()
{
	...
	while (true)
	{
		...
		case 6: if (size) save(base, size); break;
		case 7: load(&base, &size, &max_size); break;
		default:
			free(base);
			return 0;
		}
...
void save(struct Employee* arr, int size)
{
	...
	fwrite(arr, sizeof(struct Employee), size, file);
	fclose(file);
}
void load(struct Employee** arr, int* size, int* max_size)
{
	...
	{
		free(*arr);
		*max_size = *size + (ARRAY_GROWTH - *size % ARRAY_GROWTH);
		*arr = (struct Employee*)malloc(sizeof(struct Employee) * *max_size);
	}
	fread(*arr, sizeof(struct Employee), *size, file);
	fclose(file);
}